...
// Apply gravity
if (d_gravity != 0) {
    var grav_hvec = lengthdir_x(delta(d_gravity), gravity_direction);
    var grav_vvec = lengthdir_y(delta(d_gravity), gravity_direction);
    spd_hvec += grav_hvec;
    spd_vvec += grav_vvec;
}
...
if (d_friction != 0) {
    if (d_speed > 0) {
        d_speed = max(0, d_speed - delta(d_friction));
    } else if (d_speed < 0) {
        d_speed = min(d_speed + delta(d_friction), 0);
    }
}

// Apply properties
var __d_stepFactor = delta(1);
