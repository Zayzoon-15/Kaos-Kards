...
function effectStar(_x,_y,_amount = 5,_sound = true,_xOffset = 30,_yOffset = 30,_width = sprite_width,_height = sprite_height)
{
    //Play Sound
    ...
    for (var i = 0; i < _amount; i++) {
		...
		//Create Effect
    	instance_create_layer(_x+_xOff,_y+_yOff,"Effects",oEffectStar,{
            dir : _dir
        });
    }
}
...
///@arg {real} _width The width (Default = sprite_width)
///@arg {real} _height The height (Default = sprite_height)
function effectHealth(_x,_y,_amount = 5,_sound = true,_xOffset = 30,_yOffset = 30,_width = sprite_width,_height = sprite_height)
{
    //Play Sound
...
function effectNumber(_x,_y,_value)
{
    instance_create_layer(_x,_y,"Effects",oEffectNum,{
        value : _value
    });
}
...
function effectExplosion(_x,_y,_sound = true,_scale = 2)
{
	//Play Sound
	if _sound then audioPlaySfx(snExplosion,.9,1.1);
	
	//Create Explosion
    instance_create_layer(_x,_y,"Effects",oEffectExplosion, {
		image_xscale : _scale,
		image_yscale : _scale
	});
}